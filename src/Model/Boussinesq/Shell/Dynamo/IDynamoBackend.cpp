...
namespace Dynamo {
   ...
   void IDynamoBackend::stencil(SparseMatrix& mat, const SpectralFieldId& fieldId, const int l, const Resolution& res, const bool makeSquare, const BcMap& bcs, const NonDimensional::NdMap& nds) const
   {
      ...
      else if(fieldId == std::make_pair(PhysicalNames::Magnetic::id(), FieldComponents::Spectral::POL))
      {
         if(bcId == Bc::Name::Insulating::id())
         {
            SparseSM::Chebyshev::LinearMap::Stencil::InsulatingShell bc(nN, nN-s, ri, ro, l);
            mat = bc.mat();
         }
         else
         {
            throw std::logic_error("Galerkin boundary conditions for Magnetic Poloidal component not implemented");
         }
      }
